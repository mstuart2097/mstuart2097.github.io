...
\begin{framed}
\textbf{Maximum Model}: The model with all of the explanatory variables, potentially including polynomial and interaction terms, that we would want to include in our regression model
\end{framed}
...
Reconsider the \texttt{Boston} dataset in the \texttt{MASS} package where we wish to fit a model predicting the median home value of Boston homes in the suburbs.  There are 12 potential numeric explanatory variables we can use in our model.  Determine if we should include any polynomial terms in our maximum model.
...
If adding a polynomial or interaction term appears borderline (i.e. not sure if we should add the term or not), \textbf{add the term into the maximum model}.  This decreases the chances of making Type II Error.  Excluding these significant terms will \textbf{introduce bias} into our model. 

However, the possibility of \textbf{overfitting} is possible in our maximum model.  Overfitting occurs either when 
...
We will typically want to choose the model that either \textbf{maximizes} the adjusted $r^2$ or \textbf{minimizes} the AIC or Mallow's $C_p$.  Most often, we choose Mallow's $C_p$ since that method simplifies the decision about how many predictors to include in the model
...
2.  For each of the possible variable to be removed from the model
  ...
  - Calculate Mallow's $C_p$ when adding each variable individually to the model
  
  - Add the variable to the model that has the lowest Mallow's $C_p$
  
3.  Repeat (2) for each variable not already in the model

4. If the Mallow's $C_p$ does not decrease any further, do not add any additional variables to the model, and return the final model.

### Example

Determine the ``best'' model for the \texttt{Boston} dataset from our maximum model using backward selection.
...
2.  For each of the possible variables to be removed from the model
  ...
  - Remove the variable from the model that has the biggest impact on the model (i.e. largest increase in adjusted $r^2$, largest decrease in AIC/Mallow's $C_p$)
...
Determine the ``best'' model for the \texttt{Boston} dataset from our maximum model using stepwise selection.
