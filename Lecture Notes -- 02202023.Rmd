...
Now, what if we can take both of these explanatory variables, advertising and square footage, and use them **both** in conjunction to make a linear prediction of the net sales of the franchise.
...
**Multiple Linear Regression**: An extension of simple linear regression into where we can estimate a response variable $Y$ based on multiple explanatory variables $X_1,X_2,\dots,X_{p-1}$. $p-1$ is the number of explanatory variables in our linear structure.
...
```{r}
# Advertising + Sqft because they are added together in our linear model
mod <- lm(NetSales ~ Advertising + SqFt,allgreen)
summary(mod)
```
$\hat{NetSales} = -94.49 + 28.33 Advertising + 45.59 SqFt$
...
- Interpret the intercept of the least squares regression model.

-- When money spent on advertising is equal to zero dollars and the square footage of the store is zero sq. ft., the predicted net sales for the AllGreen store is -\$94,490.

- Interpret the parameter of the regression model associated with advertising.

-- When money spent on advertising increases by \$1,000, the predicted net sales for the AllGreen franchise increases by \$28,330, holding square footage constant.

- Interpret the parameter of the regression model associated with square footage.

-- When the square footage of the store increases by 1,000 square feet, the predicted net sales for the AllGreen franchise increases by \$45,590, holding advertising constant.
...
```{r}
plot(mod,1) # Plots the residuals of the multiple linear regression model against their fitted values
plot(mod,2)
```

- The assumption of homoscedasticity/common variance is fairly subjective in this problem.  Some may see points jumbled together in certain areas, suggesting the assumption is violated.  Others may see the red line close to zero and not as bad of a spread, meaning the assumption is not violated.

- The points along the QQ-plot do not seem to deviate very far from the 45-degree between -2 and 2 on the x-axis.  Therefore, the assumption of normally distributed residuals does not appear to be violated.

## Making predictions and confidence/prediction intervals

### Example

Calculate a prediction, 95% confidence interval, and a 95% prediction interval for the net sales for stores at 3500 square feet and that spend 10,000 dollars on advertising.

- For simple linear regression, you would use the predict function, and write code for a new data frame with the inputted value of the explanatory variable.

- For multiple linear regression, you do the same thing, except now your data frame will have multiple explanatory variables!

```{r}
## The following lines of code produce a data frame with two variables, advertising and square footage
newdata <- data.frame(Advertising = 10, SqFt = 3.5)
# Once you have defined the data frame, getting the intervals is the same as simple linear regression!
predict(mod,newdata,interval="confidence")
predict(mod,newdata,interval="prediction")
```
95% CI: (323.97, 372.69), We are 95% confident that the average net sales for all AllGreen stores with 10,000 dollars spent in advertising and a store that is 3500 square feet is between \$323,968 and \$372,688.

95% PI: (240.85,455.81), We are 95% confident that a randomly selected AllGreen store with 10,000 dollars spent in advertising and at 3500 square feet is between \$240,851 and \$455,806.
...
Compare the $r^2$ from a simple linear regression where we predict AllGreen Net sales by advertising spending vs. a multiple linear regression where we predict AllGreen net sales by advertising spending and square footage of the store.

```{r}
mod_1 <- lm(NetSales ~ Advertising, allgreen)
mod_2 <- lm(NetSales ~ Advertising + SqFt,allgreen)
summary(mod_1)$r.squared
summary(mod_2)$r.squared
summary(mod_2)
summary(mod_1)
```
83.54% of the variation in AllGreen's net sales can be explained through its linear relationship with Advertising.

93.56% of the variation in AllGreen's net sales can be explained through a linear model with advertising spending and square footage of the store as explanatory variables.

- Adding additional explanatory variables in our linear model will ALWAYS increase the r-squared.

```{r}
# How to easily create a multiple linear regression model for a response where all of the other variables in the dataset are included as explanatory variables
mod_all <- lm(NetSales ~ ., allgreen)
# The period tells R to include all other variables in the dataset as explanatory variables
summary(mod_all)
```
For the linear model predicting net sales where every other variable in the dataset is an explanatory variable: $\hat{NetSales} -18.85 + 16.20Sqft + 0.1746Inventory + 11.53 Advertising + 13.58 SizeofDist - 5.311 Stores$.

- Interpret the parameter associated with Size of District in the context of the problem: For every 1000 families in the district, the expected/predicted net sales for the AllGreen franchise increases by 13,580 dollars, holding all other explanatory variables constant.

- Make a prediction for net sales at an AllGreen store with 4000 square feet of store, 200,000 dollars of inventory, 12,000 dollars spent on advertising, a district of 6500 families, and 5 competing stores in the area.

```{r}
-18.85 + 16.2*4 + 0.1746*200 + 11.53*12 + 13.58*6.5 - 5.311*5
newdata <- data.frame(SqFt = 4,Inventory = 200, Advertising = 12, SizeofDist = 6.5,NoofStores = 5)
predict(mod_all,newdata)
```
