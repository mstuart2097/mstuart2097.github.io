...
Note that, for $s^2$ we divide the sum of squared errors by $n-2$, whereas for $s_Y^2$ we divide the sum of squared errors by $n-1$.
...
\textbf{Residual Plot:} A plot of the residuals from a least squares regression model, plotted against the explanatory variable $X$.

```{r}
```
Want to see either no pattern at all, or some type of random scatter around a horizontal line at zero.  We also want to see the same type of spread of the residuals across different values of $x$.
...
\textbf{Quantile-Quantile (QQ) Plot:} A plot that visualizes the residuals from a model against an assumed error distribution (normal distribution for the least squares model).
...
We want to see points that fall closely to a 45-degree line.
