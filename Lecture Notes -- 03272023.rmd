...
$\hat{medv} = e^{3.125 - 0.025 crim}$

- In general: if we perform a log-transformation on the response variable, $\log(\hat{y}) = \hat{\beta}_0 + \hat{\beta}_1 x$, $\hat{y} = e^{\hat{\beta}_0 + \hat{\beta}_1 x}$

- $100\times (e^{\hat{\beta}_1} - 1)$ represents the percent change in the predicted response for a one unit increase in $x$

- For the Boston dataset problem, when crime rate increases by 1 percent, the predicted median home value changes by $100(e^{-0.025} - 1)$ percent.
...
```{r}
mod_red <- lm(log(medv) ~ 1,Boston)
mod_full <- lm(log(medv) ~ crim,Boston)
anova(mod_red,mod_full)
```
$F = 194.8$, $p < 2.2 \times 10^{-16} < \alpha$, so we reject $H_0$ and we conclude that there is a statistically significant linear relationship between crime rate and log-median home value in Boston neighborhoods.

d. What is the predicted median home value in a Boston neighbor hood with a crime rate of 0.5.

- Use the predict function!
```{r}
newdata <- data.frame(crim = 0.5)
logpred <- predict(mod_full,newdata)
exp(logpred) # exponential funcion
```
22,480 dollars.
...
- The graphs look better, but still some concern about whether or not the assumptions are met.

```{r}
plot(Boston$crim,log(Boston$medv))
```
...
a.  Write out the least squares regression line:
```{r}
mod_full <- lm(log(medv) ~ crim + I(crim^2),Boston)
# I is how you tell R to include a polynomial term
summary(mod_full)
```
$\log(\hat{medv}) = 3.174 - 0.0528crim + 0.0004847crim^2$
...
- Reduced Model: $\log(\hat{medv}) = \beta_0$

- Full Model: $\log(\hat{medv}) = \beta_0 + \beta_1 crim + \beta_2 crim^2$
...
anova(mod_red,mod_full)
```
$F = 146.5$, $p < 2.2 \times 10^{-16} < \alpha$, so we reject $H_0$ and conclude that adding a linear and quadratic term for crime rate significantly improves the predictive ability of log-median home value.
...
anova(mod_red,mod_full)
```
$F = 71.12$, $p = 3.57 \times 10^{-16} < \alpha$, reject $H_0$, we conclude that adding a quadratic term for crime rate in a model predicting log median home value when a linear term for crime rate is included is statistically significant.
