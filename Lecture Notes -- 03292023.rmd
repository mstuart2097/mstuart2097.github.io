...
- If the plot looks non-linear, add a quadratic, cubic, or higher-order polynomial term to your model, which ever you think looks appropriate (choice of adding polynomial terms to a model is fairly subjective)
...
```{r}
library(MASS)
plot(Boston$crim,Boston$medv) # Add quadratic term for crime rate
plot(Boston$zn,Boston$medv) # No pattern, but should still include
plot(Boston$lstat,Boston$medv) # Add quadratic term for lstat
# All other plots do not appear to be non-linear in nature
```
...
summary(mod_max)
```
- Some of the variables are not statistically significant (p-values in the model summary).  How do we choose which variables to eliminate?
...
- F-statistic: $F_p = \frac{\text{SSE}(p) - \text{SSE}(k)/(k-p)}{\text{SSE}(k)/(n-(k+1))} = \frac{\text{SSE}(p) - \text{SSE}(k)/(k-p)}{\text{MSE}(p)}$
  - Can compute a p-value for $F_p$ using an F-distribution with $k-p$ and $n-(k+1)$ degrees of freedom used to determine if an explanatory variable is statistically significant for a given model
...
1.  Start with a model that contains all possible predictors (i.e. $Y \sim \mathcal{N}(\beta_0 + \beta_1X_1 + \cdots + \beta_{p-1}X_{p-1},\sigma^2)$)
...
```{r}
# step performs variable selection procedures for regression models
mod_best <- step(mod_max,direction = "backward")
```
- At the first step, industry is removed because it lowers AIC the most.  After that, removing another variable will not reduce the AIC any further, so we have obtained our "best" model.
```{r}
summary(mod_best)
```
...
```{r}
mod_best_step <- step(mod_max,direction="both") # Stepwise Selection
```
...
- The residuals look fairly evenly spread, so the assumption of homoskedasticity does not appear to be violated.

- The points fall fairly close to the 45 degree line in the QQ plot, so the assumption of normality does not appear to be violated.

*Note*: When you are choosing your best model for the exam, start with the actual response variable.  Then, if the assumptions of normality or homoskedasticity are not met, try a transformation of the response variable.
