...
Recall from Chapter 3, we said that $$\frac{\bar{Y} - \mu}{\frac{s_Y}{\sqrt{n}}} \sim t_{df=n-1}$$ where $\bar{Y}$ is the estimator of $\mu$.  We can obtain a similar conclusion regarding the distribution of $\hat{\beta}_0$ and $\hat{\beta}_1$.  $$\frac{\hat{\beta}_0  - \beta_0}{s_{\hat{\beta}_0}} \sim t_{df = n-2}$$ and$$\frac{\hat{\beta}_1 - \beta_1}{s_{\hat{\beta}_1}} \sim t_{df = n-2}$$ where $s_{\hat{\beta}_0}^2 = s^2\left( \frac{1}{n} + \frac{\bar{x}^2}{(n-1)s_X^2}\right)$ and $s_{\hat{\beta}_1}^2 =  \frac{s^2}{(n-1)s_X^2}$ (Note: Often times, inference on $\beta_0$ is not meaningful to us.).  The degrees of freedom is $n-2$ because we have to estimate two parameters $\beta_0$ and $\beta_1$.
...
se <- 0.2102
alpha <- 0.1
n <- nrow(bloodpressure) # nrow counts the number of rows in a data frame or a matrix
crit <- qt(1-alpha/2,df=n-2)
est + c(-1,1)*crit*se
```
(0.6133,1.328).  We are 90% confident that when a person's age increases by 1 year, their predicted systolic blood pressure increases by between 0.6133 and 1.328 mm/Hg.
...
```{r, echo=FALSE}
plot(SBP ~ Age,data = bloodpressure)
abline(lm(mod)) # Plots the least squares regression line from our linear model
abline(h=mean(bloodpressure$SBP),lty=2) # Plots a horizontal line at the sample mean of systolic blood pressure, lty = 2 tells us to make it a dashed line
```
...
Test Statistic: $$t = \frac{\hat{\beta}_1}{s_{\hat{\beta}_1}}$$.  Distribution of the test statistic under $H_0$ is a $t$-distribution with $n-2$ degrees of freedom.  This is the distribution that we use to calculate the corresponding p-value.

p-values: Note that $T$ represents a $t$-distributed random variable with $n-2$ degrees of freedom.

Decision: 

- If $p-value \leq \alpha$, reject $H_0$.  We have statistically significant evidence that $X$ is significant in predicting $Y$.

- If $p-value > \alpha$, do not reject $H_0$.  We do not have statistically significant evidence that $X$ is significant in predicting $Y$.
...
Full Model: $\hat{SBP} = \beta_0 + \beta_1Age$
...
summary(mod)
```
$t = 4.618$ and $p = 7.87 \times 10^{-5}$. $p \leq \alpha$, so we reject $H_0$, and we conclude that there is a statistically significant linear relationship between age and systolic blood pressure.
...
```{r}
drug <- read.csv("../Data/drugconcentration.csv")
plot(DrugConc ~ Hours,data=drug)
```

- The relationship between number of hours and concentration of the drug in the body appears to be non-linear (quadratic,exponential).

Now, let's look at the results of the linear model regressing drug concentration on number of hours.

```{r}
mod2 <- lm(DrugConc ~ Hours,data=drug)
summary(mod2)
```
- If we were to test for a significant linear relationship between hours and drug concentration, our p-value is 0.00515, meaning, at $\alpha = 0.05$, we would reject $H_0$ and say the relationship is significantly linear.

- Just because a hypothesis test provides significant evidence that $X$ and $Y$ are linear does not mean that they are!

### Confidence Intervals for $\mu_{Y|X}$ for $X = x_0$

Suppose we are interested in inference for $\mu_{Y|X = x_0}$, the mean of $Y$ for all members of the population for a given value of $X$ ($x_0$).  We have already shown an estimate of $\mu_{Y|X = x_0}$, $$\hat{Y}_{x_0} = \hat{\beta}_0 + \hat{\beta}_1 x_0.$$  We can also say that $$\frac{\hat{Y}_{x_0} - \mu_{Y|x_0}}{s_{\hat{Y}_{x_0}}} \sim t_{df=n-2}$$ where $s_{\hat{Y}_{x_0}}^2 = s_{Y|X}^2\left(\frac{1}{n} + \frac{(x_0 - \bar{x})^2}{(n-1)s_X^2}\right)$.  Therefore, we can calculate a $C = 100\times(1-\alpha)$% confidence interval for $\mu_{Y|x_0}$ as $$\hat{Y}_{x_0} \pm t_{1-\frac{\alpha}{2},n-2}\times s_{\hat{Y}_{x_0}}.$$
...
```{r}
# Recall our linear model is mod
newdata <- data.frame(Age=55)
predict(mod,newdata,interval="confidence",level=0.9)
# fit is the value directly from the least squares regression line
# lwr is the lower bound for the interval
# upr is the upper bound for the interval
```
(145.68,154.54) We are 90% confident that the mean systolic blood pressure for all 55 year olds is between 145.68 and 158.54 mm/Hg.
...
```
(121.97,182.26) We are 90% confident that a randomly selected 55 year old has a systolic blood pressure between 121.97 and 182.26 mm/Hg.
